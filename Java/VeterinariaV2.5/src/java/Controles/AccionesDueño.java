...
public class AccionesDueño {
    ...
    // New Register
    public static int nuevoRegistro(Mascota e, Dueño d) {
        int status = 0;
        try {
            ...
            String n = "insert into dueño(correo, nom, appat, apmat, dir, password,idMascota) "
                    + "values(?,?,?,?,?,?,last_insert_id()";

            PreparedStatement psd = con.prepareStatement(n);

            psd.setString(1, d.getCorreo());
            psd.setString(2, d.getNom());
            psd.setString(3, d.getAppat());
            psd.setString(4, d.getApmat());
            psd.setString(5, d.getDir());
            psd.setString(6, d.getPassword());

            status = psd.executeUpdate();
